...
from keras.layers import Dense, Dropout, Input, Embedding, concatenate, \
    GlobalAveragePooling1D, Conv1D, GlobalMaxPooling1D, TimeDistributed, BatchNormalization, LSTM, Bidirectional, Lambda
...
text_att = Attention(n_head=4, d_k=10)([em_text, em_text, em_text])  # 20,10 50,10
# text_att = BatchNormalization()(text_att)
text_att1 = Attention(n_head=4, d_k=10)([text_att, text_att, text_att])
...
plt.plot(epoch, acc_test, label='test_acc')
plt.xlabel("epoch")
