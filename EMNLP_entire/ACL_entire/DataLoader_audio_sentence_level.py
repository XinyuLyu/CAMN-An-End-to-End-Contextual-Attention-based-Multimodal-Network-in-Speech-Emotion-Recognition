...
from data import load_data_audio, save_data_audio, save_data_audio_wav, load_data_audio_wav, \
    save_data_audio_mfsc_sentence, load_data_audio_mfsc_sentence
...
reg_path = r'E:\Yue\Entire Data\iemocap_ACMMM_2019\label_multi.npy'
...
def separate_regression_label(path, labels):
    data = np.load(path)
    ang = []
    hap_exc_sur = []
    sad_fea_dis = []
...
def separate_dataset(audio_data, text_data, label):
    ...
    ang_reg, hap_exc_reg, sad_reg, fru_neu_reg = separate_regression_label(reg_path, label)
    ang_i = 0
    ...
    while hap_exc_i < len(hap_exc_audio):
        if random.randint(0, 100) < 80:
            ...
            train_label.append(hap_exc_reg[hap_exc_i])
        else:
            ...
            test_label.append(hap_exc_reg[hap_exc_i])
        hap_exc_i += 1
    ...
    while fru_neu_i < len(fru_neu_audio):
        ...
        if random.randint(0, 100) < 80:
            ...
            train_label.append(fru_neu_reg[fru_neu_i])
            ...
            test_label.append(fru_neu_reg[fru_neu_i])
...
def data_generator(path, audio_data, audio_label, num):
    ...
    while 1:
        ...
        while j < batch_size:
            ...
            j += 1
            i += 1
        yield ({'input_1': np.array(res)}, {'dense_5': np.array(res_label)})
...
def get_data():
    ...
    test_label = np.array(test_label_o)
    test_text_data = sequence.pad_sequences(test_text_data, padding='post', truncating='post', maxlen=maxlen)
    save_data_audio_mfsc_sentence(train_audio_data, train_text_data, train_label, test_audio_data, test_text_data, test_label, test_label_o)
...
def get_data1():
    dic = get_dictionary(dic_path)
    embed_matrix = initial_embed(dic, embed_path)
    train_audio_data, train_text_data, train_label, test_audio_data, test_text_data, test_label, test_label_o = load_data_audio_mfsc_sentence()
    return train_audio_data, train_text_data, train_label, test_audio_data, test_text_data, test_label, test_label_o, embed_matrix, dic
