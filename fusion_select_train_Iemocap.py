...
def get_data(path):
    ...
    print('loading data...')
    train_a = np.load(path+path1+'train_audio.npy')
    train_t = np.load(path + 'train_text.npy')
    dev_a = np.load(path+path1+'dev_audio.npy')
    dev_t = np.load(path + 'dev_text.npy')
    dev_l = np.load(path + 'dev_label.npy')
    train_l = np.load(path + 'train_label.npy')
    train_a_last = np.load(path+path1+'train_audio_last.npy')
    train_t_last = np.load(path + 'train_text_last.npy')
    dev_a_last = np.load(path + path1+ 'dev_audio_last.npy')
    dev_t_last = np.load(path + 'dev_text_last.npy')
    print('finish loading data...')

    return train_a, train_t, train_l, dev_a, dev_t, dev_l, train_a_last, \
           train_t_last, dev_a_last, dev_t_last
...
# Load the data
train_audio, train_text, train_label, dev_audio, dev_text, dev_label, train_audio_last, \
train_text_last, dev_audio_last, dev_text_last = get_data(data_path)
...
for i in range(200):
    ...
    # epo_audio, epo_text, epo_audio_last, epo_text_last, epo_label = shuffle(train_audio, train_text, train_audio_last,
    #                                                                         train_text_last,
    #                                                                         train_label)
    history = model.fit([train_audio, train_text, train_audio_last, train_text_last], train_label, batch_size=batch_size,
                        epochs=1, shuffle=True, verbose=1)
    loss_train.append(history.history['loss'])
    loss_f, mae_f = model.evaluate([dev_audio, dev_text, dev_audio_last, dev_text_last], dev_label, batch_size=batch_size,
                                   verbose=0)
    ...
    if loss_f <= loss:
        mae = mae_f
        loss = loss_f
        output_test = model.predict([dev_audio, dev_text, dev_audio_last, dev_text_last], batch_size=batch_size)
        acc = compute_acc_9class(output_test, dev_label, 6)
        model.save_weights(save_path + 'fusion_context_5_3_384_update.h5')
        # text_att_model.save_weights(save_path + 'text_att_model.h5')
        # audio_att_model.save(save_path + 'audio_att_model.h5')
